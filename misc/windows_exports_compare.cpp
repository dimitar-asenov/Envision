...
int main(int argc, char* argv[])
{
	if (argc != 3)
	{
		cout << "This tool is used to extract the difference in exported symbols between two dlls." << endl;
		cout << "You need to provide two text files as input." << endl;
		cout << "Each file is the output of 'dumpbin /exports mydll.dll'" << endl;
		return 0;
	}
	...
	cout << endl << "Lines only in " << argv[1] << endl;
	...
	for(auto line : lines1)
	{
		...
		if (!found) cout << line << endl;
	}
	
	cout << endl << "Lines only in " << argv[2] << endl;
	for (auto l : lines2) cout << l << endl;
	cout << endl << identicalLines << " identical lines" << endl;
	return 0;
}